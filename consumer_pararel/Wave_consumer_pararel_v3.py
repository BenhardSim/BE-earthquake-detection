...
def prediction_res(data_responses):
    ...
    for i in range(0,min_len_channel-window_size+1):
        ...
        all_combined_data = np.vstack((all_combined_data, normalize_data_array))
        # print(normalize_data_array)
        # input ke model 

    all_combined_data = all_combined_data.reshape(min_len_channel-window_size, WINDOW_SIZE,3)
    predictions = model.predict(all_combined_data,verbose=0)
    return predictions
    #     # predictions = 1.0
